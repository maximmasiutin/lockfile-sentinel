...
def _example_invocation() -> dict[str, object]:
    return {
        "id": "00000000-0000-0000-0000-000000000000",
        "started_utc": "2026-08-15T00:00:00Z",
        "roots": ["/t"],
        "include_node_modules": False,
        "requested_layers": ["builtin", "overlay", "osv", "trivy"],
    }
...
def test_the_json_report_is_an_envelope_not_a_bare_array() -> None:
    ...
    status = ls.RepoStatus(name="t", path="/t")
    report = json.loads(ls.render_json([status], _example_invocation(), complete=True))
    ...
    assert inv["finished_utc"] is not None
    assert inv["roots"] == ["/t"]
...
def test_a_snapshot_write_never_claims_to_be_final() -> None:
    ...
    report = json.loads(ls.render_json([], _example_invocation(), complete=False))
...
def test_snapshot_output_is_deterministic_for_a_fixed_invocation() -> None:
    """The finish stamp is the only field read from the clock at render time,
    and only on the final write, so two snapshots of the same state must be
    byte-identical; a diff between them is a change in findings, never noise."""
    status = ls.RepoStatus(name="t", path="/t")
    invocation = _example_invocation()
    first = ls.render_json([status], invocation, complete=False)
    second = ls.render_json([status], invocation, complete=False)
    assert first == second
...
def test_the_published_schema_and_example_match_what_the_code_writes() -> None:
    ...
    field renamed in the renderer and not in them would ship a contract the
    tool no longer honours. Comparing key sets pins the three together."""
    ...
    rendered = json.loads(
        ls.render_json(
            [ls.RepoStatus(name="t", path="/t")], _example_invocation(), complete=True
        )
    )
    assert list(example) == list(rendered)
    assert list(example["invocation"]) == list(rendered["invocation"])
    assert list(example["repositories"][0]) == list(rendered["repositories"][0])
