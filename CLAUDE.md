...
- The main checkout `$(MAXIM_REPOS_DIR)\lockfile-sentinel` must always stay on `master`. Never switch branches, commit, or edit files in it directly. `$(MAXIM_REPOS_DIR)` is a documentation placeholder for the `MAXIM_REPOS_DIR` environment variable, not shell syntax.
