...
- The staged Trivy databases are validated by filesystem identity before promotion: the scratch tree's device and inode are recorded when it is created and re-read immediately before it is renamed into the cache, and again after a cross-device copy, and a mismatch refuses the promotion with the live cache untouched. The identity is read without following links, so a symbolic link or junction left at the staged name is refused rather than promoted. A cross-device copy must also carry a random marker token written into the scratch just before it, since an attacker who renames the validated tree aside during the copy and back before the check restores the inode and passes a comparison but cannot reproduce a token the private directory kept unreadable. On a scratch base that grants another local account `DELETE_CHILD`, that account could rename the scanned tree aside and leave its own at the same name between the ClamAV gate and the promotion. The check narrows that window rather than closing it, since the swap can still land between the comparison and the rename.
- A promotion rename that fails for a reason other than a cross-device link is reported as it happened instead of being retried as a copy, so a permission or missing-path failure no longer half-writes the `.incoming` tree and reports its own error in place of the real cause.
