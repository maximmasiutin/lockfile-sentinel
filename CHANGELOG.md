...
- The staged Trivy databases are validated by filesystem identity before promotion: the scratch tree's device and inode are recorded when it is created and re-read immediately before it is renamed into the cache, and again after a cross-device copy, and a mismatch refuses the promotion with the live cache untouched. The identity is read without following links, so a symbolic link or junction left at the staged name is refused rather than promoted. On a scratch base that grants another local account `DELETE_CHILD`, that account could rename the scanned tree aside and leave its own at the same name between the ClamAV gate and the promotion. The check narrows that window rather than closing it, since the swap can still land between the comparison and the rename.
